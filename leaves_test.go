...
func TestMSLTR(t *testing.T) {
	...
	// loading model
	path = filepath.Join("testdata", "msltr.model")
	reader, err = os.Open(path)
	if err != nil {
		t.Skipf("Skipping due to absence of %s", path)
	}
	...
	path = filepath.Join("testdata", "msltr_1000examples_true_predictions.txt")
	reader, err = os.Open(path)
	if err != nil {
		t.Skipf("Skipping due to absence of %s", path)
	}
...
func BenchmarkMSLTR(b *testing.B) {
	// loading test data
	path := filepath.Join("testdata", "msltr_1000examples_test.libsvm")
	reader, err := os.Open(path)
	if err != nil {
...
func BenchmarkHiggs(b *testing.B) {
	// loading test data
	path := filepath.Join("testdata", "higgs_1000examples_test.libsvm")
	reader, err := os.Open(path)
	if err != nil {
...
func TestAgaricus(t *testing.T) {
	...
	// loading true predictions as DenseMat
	path = filepath.Join("testdata", "agaricus_true_predictions.txt")
	reader, err = os.Open(path)
	if err != nil {
		t.Skipf("Skipping due to absence of %s", path)
	}
